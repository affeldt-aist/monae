...
Definition actmt (X Y : UU0) : (X -> Y) -> tensorS X -> tensorS Y :=
  fun f : X -> Y => fun xs : X * S => match xs with (x, s) => (f x, s) end.

Let tensor_id : FunctorLaws.id actmt.
Proof.
by rewrite/actmt/FunctorLaws.id => B; apply boolp.funext => -[].
Qed.

Let tensor_o : FunctorLaws.comp actmt.
Proof.
by rewrite/actmt/FunctorLaws.comp => X Y Z g h; apply boolp.funext => -[].
Qed.
...
Definition actmh (X Y : UU0) : (X -> Y) -> homS X -> homS Y :=
  fun f : X -> Y => fun m : S -> X => f \o m.

Let hom_id : FunctorLaws.id actmh.
Proof. by rewrite/actmh/FunctorLaws.id => B; exact: boolp.funext. Qed.

Let hom_o : FunctorLaws.comp actmh.
Proof.
by rewrite/actmh/FunctorLaws.comp => X Y Z g h; exact: boolp.funext.
Qed.
...
Lemma tensorSmap {A B} (f : A -> B) (m : tensorS S A) :
  (tensorS S # f) m = let (a, s) := m in (f a, s).
Proof. by []. Qed.
...
Definition wBisimDS A (a b : DS A) := forall s, a s ≈ b s.

Section wBisimDS.
Local Notation "a '≈' b" := (wBisimDS a b).
...
apply: bindfwB.
by move=> [[b'|a'] s'] /=; rewrite bindretf.
Qed.

Lemma naturalityDSE {A B C} (f : A -> DS (B + A)%type) (g : B -> DS C) (a : A) :
   wBisimDS (bindS (whileDS f a) g)
            (whileDS (fun y => f y >>= sum_rect (fun => DS (C + A)) (DS # inl \o g) (DS # inr \o (@ret DS A))) a).
Proof.
...
have -> : ((homS S \o M) \o tensorS S) # sum_rect (fun=> (B + A)%type) idfun inr =
          homS S # (M # (tensorS S # sum_rect (fun=> (B + A)%type) idfun inr)).
  by rewrite -compA FCompE.
rewrite homSmap /= fmapE !bindA.
apply: bindfwB.
by move=> [[[bl|al']|al] sl]; rewrite !bindretf /= fmapE bindretf /= bindretf.
Qed.

Lemma whilewBDS {A B} (f g : A -> DS (B + A)) (a : A) :
  (forall a, wBisimDS (f a) (g a)) -> wBisimDS (whileDS f a) (whileDS g a).
Proof.
...
Lemma bindmwBDS {A B} (f : A -> DS B) (d1 d2 : DS A) :
  wBisimDS d1 d2 -> wBisimDS (d1 >>= f) (d2 >>= f).
Proof. by rewrite /wBisimDS => Hd s /=; rewrite !MS_bindE; exact: bindmwB. Qed.

Lemma bindfwBDS {A B} (f g : A -> DS B) (d : DS A) :
  (forall a, wBisimDS (f a) (g a)) ->
  wBisimDS (d >>= f) (d >>= g).
Proof.
by rewrite /wBisimDS => Hfg s /=; rewrite !MS_bindE /=; apply: bindfwB => -[].
Qed.
...
apply eq_bind.
by move=> [[?|?] ?]; rewrite /=bindretf fmapE !bindretf/= fmapE bindretf.
Qed.
