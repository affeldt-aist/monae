...
(*                        the category of semi-up_complete semi-lattice convex   *)
...
Proof. by move=> hf hg ? ? ? /=; rewrite up_compE hf hg. Qed.
...
by rewrite up_compE Convn_of_fsdistmap.
Qed.
...
Proof.
by move=> a b h; rewrite funeqE=> x; rewrite FIdf /eta0' up_compE/= fsdistmap1.
Qed.
...
Proof.
by move=> c; apply funext=> x /=; rewrite eps0E eta0E up_compE triangular_laws_left0.
Qed.

Lemma triR0 : TriangularLaws.right eta0 eps0.
Proof.
by move=> c; rewrite eps0E eta0E funeqE => a /=; rewrite up_compE Convn_of_fsdist1.
Qed.
...
move=> [bf af] [bg ag]; split => x/=.
  rewrite up_compE bf bg/=; congr (biglub _).
  by apply/neset_ext => /=; rewrite image_comp.
by move=> ? ? /=; rewrite up_compE af ag.
Qed.
...
rewrite up_compE biglub_morph; congr (|_| _).
by rewrite free_semiCompSemiLattConvType_morE.
Qed.
...
Proof. by move=> c; apply funext=> /= x; rewrite eps1E eta1E up_compE biglub1. Qed.
