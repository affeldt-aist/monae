...
(*
Definition factdts (n: nat):=
  do r <- cnew ml_int 1;
  do l <- cnew ml_int 1;
  do res <- while (fun ( _:unit) =>
        do v <- cget r;
        do i <- cget l;
        if i <= n then do _ <- cput r (i*v);
                      do _ <- cput l (i.+1); Ret (inr tt )
                 else Ret (inl v)) tt;
               Ret res.
*)

Definition factdts_body (r l: loc ml_int) (n: nat):unit -> M (unit + unit)%type  := fun _ =>
        do v <- cget r;
        do i <- cget l;
        if i <= n then do _ <- cput r (i*v);
                      do _ <- cput l (i.+1); Ret (inr tt )
...
(*
Definition factdts_body (r: loc ml_int) :nat -> M (nat + nat)%type  := fun n =>
        do v <- cget r;
        if n==0 then Ret (inl v)
                else do _ <- cput r (n*v);
                         Ret (inr (n -1)).
*)
Definition factdts (n: nat) := do r <- cnew ml_int 1; do l <- cnew ml_int 1; do _ <-  while (factdts_body r l n) tt; do v <- cget r; Ret r.
End factorial.
