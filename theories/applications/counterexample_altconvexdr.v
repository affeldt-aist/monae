...
From mathcomp Require Import boot order ssralg ssrnum finmap ssrint.
From mathcomp Require Import boolp classical_sets reals.
From mathcomp Require Import unstable. (* for onem *)
...
Local Notation "{% q }" := (q%:i01) (at level 0, format "{% q }").
...
Local Lemma qk_eqE a k :
  0 < a -> (q_ k == a) = ((ln a - ln q%:num) / (ln r%:num - ln p%:num) == k%:R).
Proof.
move=> a_gt0.
have lnrp_neq0 : ln r%:num - ln p%:num != 0.
...
Let Th := (ln p%:num - ln q%:num) / (ln r%:num - ln p%:num).
...
Let qk_prob k (kTh : k%:R <= Th) := Prob.mk (qk_prob_proof kTh).
Canonical qk_prob.
...
Proof.
rewrite (ltW qk_gt0)/=; move/qS_le_r/le_trans; apply.
exact: prob_le1.
Qed.

Let qS_prob k (kTh : k%:R <= Th) := Prob.mk (qS_prob_proof kTh).
Canonical qS_prob.

Local Lemma qk_magnifiable k (kTh : k%:R <= Th) : {%0} <= qk_prob kTh <= p.
Proof. by rewrite -!Order.le_val/= prob_ge0//= qk_leE. Qed.
...
Proof.
rewrite qS_prob_conv choice_conv conv_conv//; first exact: ge0.
...
apply: (@collapse_left y x {%r%:num.~} {%q%:num.~} {%p%:num.~}).
