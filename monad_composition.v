...
Definition prod1 := forall (A B : Type) (f : A -> B), prod \o N # ((M \O N) # f) = (M \O N) # f \o prod.
...
Definition dorp1 := forall (A B : Type) (f : A -> B), dorp \o (M \O N) # (M # f) = (M \O N) # f \o dorp.
...
Definition swap1 := forall (A B : Type) (f : A -> B), swap \o N # (M # f) = M # (N # f) \o swap .
