...
Section comp.
Variables M N : monad.
Definition fmap A B : (A -> B) -> M (N A) -> M (N B) := (@fmap _ _ _) \o (@fmap _ _ _).
...
Definition prod1 := forall A B (f : A -> B), prod \o fmap (Comp.fmap f) = Comp.fmap f \o prod.
...
Lemma JOIN_ret : join_left_unit (@JOIN) (@Comp.ret _ _).
Proof.
move=> A; rewrite /JOIN /Comp.ret /= compA -(compA join) fmap_ret.
by rewrite compA join_ret compidf Hprod2.
Qed.

Lemma JOIN_fmap_ret : join_right_unit (@JOIN) (@Comp.fmap _ _) (@Comp.ret _ _).
Proof.
move=> A.
...
Definition dorp1 := forall A B (f : A -> B), dorp \o Comp.fmap (fmap f) = Comp.fmap f \o dorp.
Definition dorp2 := forall A, (@dorp A) \o Comp.ret = fmap Ret.
Definition dorp3 := forall A, (@dorp A) \o Comp.fmap Ret = id.
Definition dorp4 := forall A, (@dorp A) \o JOIN = JOIN \o Comp.fmap dorp.
