...
- [[monad_transformer.v][monad_transformer.v]]: monad transformers
  + completed by ~fmt_lifting.v~ and ~parametricty_codensity.v~ which require disabling of universe checking
    * the directory ~impredicative_set~ contains a lighter version of Monae where monads live in ~Set~
      and that compiles with ~impredicate-set~ (and universe checking enabled)
