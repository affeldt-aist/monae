...
Lemma uniform_unfold {M : probMonad} (P : rel X) def d :
  uniform def (enum X) >>= (fun p => Ret (P d p)) =
    Ret (P d a) <|(/ 3)%:pr|> (Ret (P d b) <|(/ 2)%:pr|> Ret (P d c)) :> M _.
Proof.
...
Lemma uniform_unfold_pair {M : probMonad} def (P : rel X) :
...
Lemma bcoin13E_pair {M : probMonad} def (f : bool -> M bool) :
...
transitivity
  (uniform (def, def) (cp (enum X) (enum X)) >>= (fun hp => Ret (P hp.1 hp.2)) : M _).
  by bind_ext.
...
Definition hide {M : probMonad} : M door := uniform def doors.

Definition pick {M : probMonad} : M door := uniform def doors.

Definition tease {M : probMonad} (h p : door) : M door := uniform def (doors \\ [:: h ; p]).
...
rewrite {1}/play {1}/monty hide_pickE.
transitivity (do hp <- uniform (def, def) (cp doors doors) : M _;
  do t <- tease hp.1 hp.2; Ret ((head def (doors \\ [:: hp.2; t])) == hp.1))%Do.
  bind_ext => -[h p].
...
Lemma hide_pick_nondeter : (do h <- hide_n; do p <- pick def : M _; Ret (h == p) = uFFT)%Do.
Proof.
transitivity (fmap (uncurry (fun a b => a == b)) (do h <- hide_n; do p <- pick def : M _; Ret (h, p)))%Do.
...
transitivity (
  hide_n >>= (fun h : door => (pick def : M _) >>= (fun p : door => tease_n h p >> Ret (p == h)))
).
  by bind_ext => d; bind_ext => d'; rewrite_ bindretf.
transitivity (
  hide_n >>= (fun h : door => (pick def : M _) >>= (fun p : door => Ret (h == p)))
).
...
Lemma bcoin23E :
  arbitrary def doors >>= (fun h : door => uniform def doors >>= (fun p : door => Ret (h != p) : M _)) =
  bcoin (@Prob.mk (2/3) H23) :> M _.
Proof.
transitivity (
  arbitrary def doors >>= (fun h : door => uniform def doors >>= (fun p : door => Ret (h, p) >>= (fun x => Ret (x.1 != x.2) : M bool)))
).
  by bind_ext => h; rewrite_ bindretf.
transitivity (
  (arbitrary def doors >>= (fun h : door => uniform def doors >>= (fun p : door => Ret (h, p) : M _))) >>= (fun x => Ret (x.1 != x.2) : M bool)
).
  by rewrite bindA; rewrite_ bindA.
rewrite monty_choice_your_choice_combine /pick /monty.pick 2!alt_bindDl.
have K : forall D, (uniform def doors >>= (fun p : door => Ret (D, p))) >>= (fun x : door * door => Ret (x.1 != x.2)) = (uniform def doors >>= (fun p : door => Ret (D != p))) :> M _.
...
transitivity (
  hide_n >>=
  (fun h : door => (pick def : M _) >>= (fun p : door => tease_n h p >>= (fun t : door => Ret (h == head def (doors \\ [:: p; t])))))
).
  by bind_ext => d; bind_ext => d'; rewrite_ bindretf; rewrite_ eq_sym.
transitivity (
  hide_n >>= (fun h : door => (pick def : M _) >>= (fun p : door => tease_n h p >> if h == p then Ret false else Ret true))
).
...
transitivity (hide_n >>= (fun h : door => (pick def : M _) >>= (fun p : door => Ret (h != p)) : M _)).
...
Definition tease_f (h p : door) : M door :=
  uniform def (doors \\ [:: p]) >>= (fun t => if t == h then Fail else Ret t : M _).
...
rewrite_ bindretf.
rewrite (Set3.bcoin13E_pair _ def (fun b => if b then Ret true else (Fail : M _) <| (/ 2)%:pr |> (Ret false : M _))) //.
rewrite /bcoin.
...
Open (X in _ >>= X).
transitivity (if x.1 == x.2
  then uniform def (doors \\ [:: x.1]) >> Ret false
  else
   ((Fail : M _) <| (/ 2)%:pr |> @RET M _ (head def (doors \\ [:: x.1; x.2]))) >>= (fun x0 =>
   Ret (head def (doors \\ [:: x.2; x0]) == x.1))).
...
transitivity (uniform (def, def) (cp doors doors) >>= (fun x =>
  ...
  then Ret false
   ...
   (Fail : M _)
   <| (/ 2)%:pr |>
   (@RET M _ (head def (doors \\ [:: x.2; head def (doors \\ [:: x.1; x.2])]) == x.1)) : M _)).
...
rewrite (Set3.bcoin13E_pair _ def (fun b => if b then Ret false else (Fail : M _) <| (/ 2)%:pr |> (Ret true : M _))) //.
by rewrite prob_bindDl 2!bindretf.
(* TODO: flattening choices *)
Qed.
