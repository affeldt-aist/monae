...
Definition coin : M bool := Ret false [~i] Ret true.

Definition coin' := do x <- coin ; do y <- coin ; guard (x + y > 0) >> Ret x.

(* proof of p.419 *)
Lemma coin'E : coin' = (@Fail M _ [~i] Ret false) [~i] (Ret true [~i] Ret true).
Proof.
...
Record mixin_of (M : nondetMonad) : Type := Mixin {
  ...
  _ : forall A (a b : M A), op_share (a [~i] b) = op_share a [~i] op_share b
}.
...
Lemma share_choice A (a b : M A) : Share (a [~i] b) = (Share a : M (M A)) [~i] Share b.
...
Lemma dup_coin_letE M : @dup_coin_let M =
  (Ret (false, false) [~i] Ret (false, true)) [~i]
  (Ret (true, false) [~i] Ret (true, true)).
...
Lemma dup_coin_bindE M : @ dup_coin_bind M =
  Ret (false, false) [~i] Ret (true, true).
Proof.
...
Fail CoFixpoint zeros : M _ := do x : M _ <- Share zeros : M _; Ret 0 [~i] x.
