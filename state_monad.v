...
Module Exports.
Definition Loop (M : t) : forall A R, (A -> m M R) -> m M R :=
  let: Pack _ (Class _ (Mixin x )) := M return forall A R, (A -> m M R) -> m M R in x.
Arguments Loop {M A R} : simpl never.
...
Definition Foreach S (M : t S) : nat -> nat -> (nat -> m M unit) -> m M unit :=
  let: Pack _ (Class _ _ (Mixin x _ _ _)) := M return nat -> nat -> (nat -> m M unit) -> m M unit in x.
Arguments Foreach {S M} : simpl never.
Coercion baseType : loopStateMonad >-> stateMonad.
Canonical baseType.
Definition state_of_loopstate S (M : loopStateMonad S) :=
  MonadLoop.Pack (MonadLoop.Class (base2 (class M))).
Canonical state_of_loopstate.
End Exports.
End MonadStateLoop.
